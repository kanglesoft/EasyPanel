...
extern "C" {
#endif
#define IS_KSAPI_VERSION_COMPATIBLE(ver) (HIWORD(ver)==KSAPI_MAIN_VERSION)
...
typedef kev_result(*async_http_io)(void *async_http_arg, KOPAQUE data, void *arg, result_callback result, buffer_callback buffer);
typedef void (*async_http_close)(void *async_http_arg, int exptected_done);
...
typedef struct _kgl_push_gate kgl_push_gate;
...
typedef struct _kgl_push_gate_function {
	int64_t(*get_post_left)(kgl_push_gate *gate, KREQUEST rq);
	kev_result(*read_post)(kgl_push_gate *gate, KREQUEST rq);//, void *arg, result_callback result, buffer_callback buffer);

	void (*push_status)(kgl_push_gate *gate, KREQUEST rq, int status_code);
	KGL_RESULT (*push_header)(kgl_push_gate *gate, KREQUEST rq, kgl_header_type attr, const char *val, hlen_t val_len);
	KGL_RESULT (*push_unknow_header)(kgl_push_gate *gate, KREQUEST rq, const char *attr, hlen_t attr_len, const char *val, hlen_t val_len);
	kev_result (*push_header_finish)(kgl_push_gate *gate, KREQUEST rq);

	KGL_RESULT (*push_body)(kgl_push_gate *gate, KREQUEST rq, const char *str, int len);
	kev_result (*push_body_finish)(kgl_push_gate *gate, KREQUEST rq, KGL_RESULT result);
	
	kev_result (*push_message)(kgl_push_gate *gate, KREQUEST rq, KGL_MSG_TYPE msg_type, int32_t msg_flag, const char *msg);
	void (*release)(kgl_push_gate *gate);
} kgl_push_gate_function;

struct _kgl_push_gate {	
	kgl_push_gate_function *f;
};

typedef struct _kgl_async_context_function {

	kgl_get_variable_f get_variable;
	kev_result(*check_next)(KREQUEST rq, KCONN cn,bool need_open);
	kev_result(*open_next)(KREQUEST rq, KCONN cn, kgl_push_gate *gate, const char *queue);	
	kev_result(*redirect)(KREQUEST rq, KCONN cn, kgl_push_gate *gate, const char *target, const char *queue);
	bool(*is_next_opened)(KREQUEST rq, KCONN cn);
...
#define get_async_context(gate) (kgl_async_context *) ((unsigned char *)gate - offsetof(kgl_async_context, ug))
...
typedef struct _kgl_access
{
	...
	KF_STATUS_TYPE(*process)(KREQUEST rq, kgl_access_context *ctx,DWORD notify);
...
struct _kgl_async_upstream
{
	...
	void *(*create_ctx)();
	void (*free_ctx)(void *ctx);
	kev_result (*check)(KREQUEST rq, kgl_async_context *ctx);
	kev_result (*open)(KREQUEST rq, kgl_async_context *ctx);
	kev_result (*read)(KREQUEST rq, kgl_async_context *ctx);
	void (*close)(KREQUEST rq, kgl_async_context *ctx);
	char *(*get_post_buffer)(KREQUEST rq, kgl_async_context *ctx, int *len);
	kev_result(*post_result)(KREQUEST rq, kgl_async_context *ctx, int got);
};
